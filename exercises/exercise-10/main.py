# To run file with uvicorn: uvicorn main:app --reload
...
fake_db = [
    ...
    {
        "id": 3,
        "question": "How many nations are in the EU?",
        "answer": "27"
    }
...
@app.get("/questions/{id}/hint", tags=["OpenAI"], description="Get a hint to a question using OpenAI")
async def get_question_hint(question_id: int):
    for question in fake_db:
        if question["id"] == question_id:
            input_text = f"Give me a hint for this question: '{question['question']}'"
            response = client.responses.create(
                model="gpt-4o-mini",
                input=input_text,
                store=True,
    ...
    print(response.output_text)
    return {"input": input_text, "response": response.output_text}


@app.get("/questions/{id}/explain", tags=["OpenAI"], description="Explain the question in detail using OpenAI")
async def explain_answer(question_id: int):
    for question in fake_db:
        if question["id"] == question_id:
            input_text = f"For this question: '{question['question']}', Explain this answer in more detail: '{question['answer']}'"
            response = client.responses.create(
                model="gpt-4o-mini",
                input=input_text,
                store=True,
